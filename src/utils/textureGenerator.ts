...
function generateNormalMap(imageData: ImageData, settings: TextureSettings['normal']): ImageData {
  ...
  for (let y = 0; y < height; y++) {
    for (let x = 0; x < width; x++) {
      ...
      if (settings.invertRed) output.data[i] = 255 - output.data[i];
      if (settings.invertGreen) output.data[i + 1] = 255 - output.data[i + 1];
    }
  }
...
function generateDisplacementMap(imageData: ImageData, settings: TextureSettings['displacement']): ImageData {
  const { width, height, data } = imageData;
  const output = new ImageData(width, height);
...
function generateAOMap(imageData: ImageData, settings: TextureSettings['ao']): ImageData {
  ...
  const output = new ImageData(width, height);
  const strength = settings.strength / 100;
  const samples = Math.round(settings.range);
...
function generateSpecularMap(imageData: ImageData, settings: TextureSettings['specular']): ImageData {
  const { width, height, data } = imageData;
  const output = new ImageData(width, height);
  const strength = settings.strength / 100;
